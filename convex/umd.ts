...
const UMDIO_BASE = "https://api.umd.io/v0";
...
/** Live umd.io adapter. NOTE: api.umd.io 502s often — callers must handle failure. */
export const umdioSource: UmdScheduleSource = {
  async fetchCourse(courseId, term) {
    ...
    const course = Array.isArray(data) ? (data[0] as UmdioCourse) : null;
  ...
  async fetchSections(courseId, term) {
    const data = await umdioGet(
      `${UMDIO_BASE}/courses/sections?course_id=${encodeURIComponent(courseId)}&semester=${encodeURIComponent(term)}&per_page=100`,
    );
