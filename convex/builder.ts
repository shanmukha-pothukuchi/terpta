...
/**
 * Point every auto-span window at the hours its TAs actually offered.
 *
 * A coordinator has to open the windows before anybody has answered the
 * availability form, so the hours are a guess — and the guess is expensive
 * in one direction: time painted outside a window is invisible, and a TA who
 * offered five to six on a window that shuts at five is simply not scheduled
 * for it, with nothing on the board to say why. A window marked `autoSpan`
 * takes the earliest start and latest end anybody painted on its weekday
 * instead, re-read on every generate as more forms come in.
 *
 * Only time a TA offered counts: "unavailable" is painted to carve holes out
 * of a day, and stretching a window to cover one would be the opposite of
 * what it says. A weekday nobody offered leaves its window untouched rather
 * than collapsed, so a period generated before anyone answers still works.
 */
export const refreshAutoWindows = internalMutation({
  args: {
    periodRef: v.id("staffingPeriods"),
    /**
     * Turn every office-hours window in the period over to following the
     * TAs, then refresh it. The one-time switch for a period whose windows
     * were opened by hand before anybody had answered.
     */
    adoptAll: v.optional(v.boolean()),
  },
  returns: v.array(
    v.object({
      day: dayValidator,
  ...
  handler: async (ctx, args) => {
    ...
    const dutyTypes = await ctx.db
      ...
      .withIndex("by_period", (q) => q.eq("periodRef", args.periodRef))
      .collect();
    const windowDuty = new Set(
      dutyTypes.filter((d) => d.mode === "window").map((d) => d._id as string),
    );
    const auto = shiftDocs.filter(
      (s) =>
        s.windowRef === undefined &&
        s.day !== undefined &&
        s.startMin !== undefined &&
        s.endMin !== undefined &&
        (args.adoptAll === true
          ? windowDuty.has(s.dutyTypeRef as string)
          : s.autoSpan === true),
    );
    ...
    const profiles = await ctx.db
      ...
      .withIndex("by_period", (q) => q.eq("periodRef", args.periodRef))
      .collect();
    const span = new Map<Day, { startMin: number; endMin: number }>();
    for (const profile of profiles) {
      const blocks = await ctx.db
        .query("availabilityBlocks")
        .withIndex("by_profile", (q) => q.eq("taProfileRef", profile._id))
      ...
      for (const b of blocks) {
        ...
        const at = span.get(b.day);
        if (!at) span.set(b.day, { startMin: b.startMin, endMin: b.endMin });
        else {
          at.startMin = Math.min(at.startMin, b.startMin);
          at.endMin = Math.max(at.endMin, b.endMin);
        }
      }
    }
    ...
    const changed = [];
    for (const w of auto) {
      if (args.adoptAll === true && w.autoSpan !== true) {
        await ctx.db.patch(w._id, { autoSpan: true });
      }
      const at = span.get(w.day!);
      if (!at || at.endMin <= at.startMin) continue;
      if (w.startMin === at.startMin && w.endMin === at.endMin) continue;
      changed.push({
        day: w.day!,
        fromStartMin: w.startMin!,
        fromEndMin: w.endMin!,
        toStartMin: at.startMin,
        toEndMin: at.endMin,
      });
      await ctx.db.patch(w._id, { startMin: at.startMin, endMin: at.endMin });
    }
    return changed;
  },
});
...
export const generate = action({
  ...
  handler: async (ctx, args): Promise<SolveDiagnostics> => {
    // Before reading anything: a window that follows its TAs has to be
    // pointed at the availability as it stands now, not as it stood when
    // the window was opened.
    await ctx.runMutation(internal.builder.refreshAutoWindows, {
      periodRef: args.periodRef,
    });
